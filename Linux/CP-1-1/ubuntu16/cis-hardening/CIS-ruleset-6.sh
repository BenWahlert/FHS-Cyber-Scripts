...
rule-6.2.16()
{
    ...
    if [ -e $flagfile ]; then
        echo "Ensure no duplicate UIDs exist - requires manual configuration"
        rm $flagfile
    fi
}
...
rule-6.2.17()
{
    ...
    if [ -e $flagfile ]; then
        echo "Ensure no duplicate GIDs exist - requires manual configuration"
        rm $flagfile
    fi
}
...
rule-6.2.18()
{
    ...
    if [ -e $flagfile ]; then
        echo "Ensure no duplicate user names exist - requires manual configuration"
        rm $flagfile
    fi
}
...
rule-6.2.19()
{
    ...
    if [ -e $flagfile ]; then
        echo "Ensure no duplicate group names exist - requires manual configuration"
        rm $flagfile
    fi
}
...
rule-6.2.20()
{
    ...
    if [ -n "$usr_from_group"  ] ||\
       ...
       echo "Ensure shadow group is empty - requires manual configuration"
    fi
}
