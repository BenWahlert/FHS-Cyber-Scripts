...
rule-4.1.1.1()
{
    ...
    if [ -n "$value" ]; then
        sed -E -i "s@(max_log_file\s+=).*@\1 $value@" /etc/audit/auditd.conf
    fi
    sed -E -i "s@(max_log_file_action\s+=).*@\1 rotate@" /etc/audit/auditd.conf
}
...
execute_ruleset-4()
{
    ...
    rulehash[lvl2_server]="${rulehash[lvl1_server]}"" 4.1.1.1 4.1.1.2 4.1.1.3 4.1.2 4.1.3\
        4.1.4-18"
    rulehash[lvl1_workstation]=$common
    rulehash[lvl2_workstation]="${rulehash[lvl1_workstation]}"" 4.1.1.1 4.1.1.2 4.1.1.3\
